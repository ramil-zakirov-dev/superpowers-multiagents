...
!`python "${CLAUDE_PLUGIN_ROOT}/scripts/orchestrator.py" milestone new --id "$(echo "$ARGUMENTS" | cut -d' ' -f1)" --title "$(echo "$ARGUMENTS" | cut -s -d' ' -f2-)"`
...
If no title was given, the brief has an empty one — say so and offer to set it.
The orchestrator accepts an empty title, so nothing else will point this out.
