...
| Situation | Behaviour |
| :--- | :--- |
| `new-milestone` invoked with no argument | `$ARGUMENTS` substitutes empty; `milestone new` validates the id and refuses: `Error: milestone_id '' contains invalid characters.` Fail-closed with a message. The only command of the eight that behaves as the original draft of this table claimed they all did. |
| `new-milestone` invoked with an id but no title | The brief is created with an **empty** title, and the orchestrator does not object. `cut -s` is what makes it empty: without `-s`, `cut` returns the whole line when it finds no delimiter, so the title would silently become a copy of the id — a plausible-looking fabrication in a tracked document, which is worse than a visibly blank field. Requiring a title is orchestrator behaviour and out of scope per §2.2; the command's prompt text tells the session to report the empty title instead. |
| Any other command invoked with no argument | `$1` substitutes empty, `Path("").resolve()` yields the working directory — which *exists* — and both families then try to read a directory as a file. **Raw traceback, not a clean refusal.** Note this defeats `dispatch-agent`'s existence check at `orchestrator.py:260` as thoroughly as it defeats `cmd_set_status`, which has none: the guard tests the wrong thing for an empty argument. |
| One of the five `set-status` commands given a path that does not exist | `cmd_set_status` reads the file with no existence check, so an unhandled `FileNotFoundError`. `dispatch` in the same situation refuses cleanly with `Error: Target file '…' not found.` **This is the real asymmetry**, and it is a pre-existing gap in `cmd_set_status`, not one this slice introduces or is permitted to fix — §2.2 forbids new orchestrator behaviour. Recorded as debt: a future slice should give `cmd_set_status` the existence check `cmd_dispatch_agent` already has. Until then, those five commands' prompt text ("the orchestrator refused and its message says why … do not retry") is optimistic here — a traceback is not a message that says why, and a model reading one is more likely to do exactly what that paragraph forbids. |
